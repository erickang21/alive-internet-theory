...
- **Analyzer unavailable:** a request to `127.0.0.1:5000` failed, or two checks in a row went unanswered, so nothing is analyzing the video (see Troubleshooting). **Try again** re-asks, and the card returns to the placeholder by itself once the backend is back.
