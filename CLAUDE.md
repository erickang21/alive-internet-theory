...
backend/analyze.py         CLI: resolve targets → download → transcript → score → store → fact check
backend/factchecks.py      Runs the fact check AFTER the evaluation is stored, and patches the row
...
- **Auto-analyze** (`content/autoAnalyze.js`, `shared/autoAnalyze.js`, `MESSAGE_TYPES.QUEUE_ANALYSIS`): the one thing that *does* POST from a feed, behind the `chrome.storage.local["aitAutoAnalyze"]` switch. An `IntersectionObserver` (200px margin) queues each tile **as it scrolls into view**, so the work is bounded by what you actually look at rather than by how far YouTube has lazy-loaded; ids are sent 3 at a time and each one only once per time the mode is on. The background handler is deliberately *not* `requestEvaluation`: it must not touch the toolbar icon, which describes the watched video, not the thirty tiles scrolling past it. Videos still `indexing` are re-asked every 15s — that, not a DOM change, is what turns a queued tile into a flagged one — and a real score is handed to `scores.primeScore` so the next rescan marks the tile immediately instead of waiting out that module's 60s miss TTL. **The watched video always comes first**: `holdFeed()` on every change of video stops the queue *and drops what the page just left had collected* (those tiles are gone), and `releaseFeed()` runs only once the video has a verdict or has definitively failed — or immediately on a page with no video to wait on. Without it a watch page's up-next tiles take slots from the video the viewer is staring at a skeleton for. In-flight sends are not cancelled on navigation, since they cannot be recalled; they just unwind the in-flight count when they settle. Its only visible effect is through the flags, so with **AI flags** off it fills the database silently.
- **Fact check** (`ui/FactCheck.jsx`, `content/factCheckBridge.js`, `shared/factCheckState.js`): the panel's **Fact check** tab, between Breakdown and Settings. It is a tab rather than a card section because the backend now runs the check after storing the evaluation, so it is routinely still loading while the verdict beside it is final — and a loading state needs somewhere of its own to live. While it loads it renders a scaffold of the report that's coming (`ui/bones.jsx`, the same wave as the verdict card's skeleton) under a line that names the phase: "Checking whether this video makes claims worth verifying…" until the pre-gate decides, then "Currently referencing citations…" once it has. It never shares the score poll above — that poll's whole design is to stop the moment a verdict settles, minutes before a fact check has anything to say — so the bridge runs its own 30s loop against `GET /video/fact-check` (via the worker's `GET_FACT_CHECK` handler) and stops on any terminal stage: `complete`, `skipped_fiction` (a genuine pre-gate verdict), or `failed` (a 404 keeps polling as `fact_checking`, an `unavailable` answer lands as `failed` with a retry). The bridge writes stage records into `chrome.storage.local` and `FactCheck.jsx` renders from them through `shared/factCheckState.js`, which is what lets a result written minutes after the verdict appear without a reload; `idle` (no record yet) is itself a wait — the bridge writes its first record on the next tick — so it scaffolds too rather than rendering a blank tab. Everything shown is model output or scraped page text, so it all renders through JSX text nodes, and citation hrefs pass an http/https allow-list first.
...
| Upload frequency + video length | up to −10 | Exact upload timestamps of the channel's latest 10 uploads via yt-dlp (see yt-dlp section) |
...
**It runs off the critical path (`backend/factchecks.py`).** It used to be step 4
of 7 inside `evaluate_video`, so the verdict card could not appear until minutes
of per-claim LLM calls had finished — for a criterion that is never scored.
Now `analyze_video` stores the evaluation first and hands the video to
`factchecks.dispatch`, which runs the check on its own thread (its own
`MAX_CONCURRENT_CHECKS` semaphore, separate from `indexing.MAX_CONCURRENT_ANALYSES`
— that one is sized to keep yt-dlp off YouTube's bot radar, and this work touches
YouTube not at all) and then patches the finished entry and the
`is_educational`/`thesis`/`hallucinated`/`validity_score`/`validity_rating` columns
into the stored row. **Nothing in the evaluation waits for it or fails because of
it**: a check that raises leaves the row exactly as the main analysis wrote it.
`scoring/fact_check.py` is split so the two phases can be reported separately —
`gate_video` (cheap: transcript length, then YouTube's categories, then at worst
one LLM call) and `run_engine` (minutes) — with `score_transcript` still composing
them for the CLI and its tests. The CLI passes `background=False` and runs the
check inline, because a script that exits after its last video would take a
background thread down with it.

`GET /video/fact-check` therefore has a fourth answer: `{"status": "running",
"phase": …}` while a job is in flight, where the phase is named as one of the
frontend's own stages (`checking_eligibility`, then `fact_checking`) so the
service worker passes it straight through. The phase lives in memory, like
`indexing`'s job table, so a backend restart mid-check loses it and the row keeps
whatever it had; nothing retries on its own.
...
- Exact timestamps come from a **full extraction of each upload** (1.2–2.4s each, measured on different days): the latest 10 (`RECENT_UPLOADS`) for cadence (median gap), plus the oldest for account age, so 11 extractions. **The loop is sequential and it is the bottleneck for any channel not already cached** — measured on stored rows, a video from a new channel took a median 46s of wall clock against 18s for a cached one. It was 19+1 extractions until this was measured; cadence takes a median gap and `scoring/youtube.py` needs 5 timestamps, so 10 keeps the statistic and halves the wait. The listing walk itself takes 7–22s for a channel with 1,848 uploads, because finding the oldest upload means reading the whole list, and no `RECENT_UPLOADS` value removes it. Uploads that fail to extract (members-only, age-restricted, removed) are skipped with a warning.
